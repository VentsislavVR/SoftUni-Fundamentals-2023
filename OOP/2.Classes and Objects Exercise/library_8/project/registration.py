...
class Registration:
    ...
    def change_username(self, user_id: int, new_username: str, library: Library) -> str:
        try:
            user = next(filter(lambda u: u.user_id == user_id, library.user_record))
        except StopIteration:
            return f"There is no user with id = {user_id}!"

        if user.username == new_username:
            return "Please check again the provided username - it should be different than the username used sofar!"
        try:
